...
$str->nav_link='hal=daftar_partatlet';
$str->edit_link='hal=tambah_partatlet';


for($i=1;$i<=2;$i++){
	
	$str->action=$_GET['action'];
	if($_GET['action']=='partisipan' and !empty($_GET['id'])){	
	$str->id=$_GET['id']; // INI id_event
	$q=mysql_query("select * from tb_event where id_event='".$str->id."'");//bener gak ni ?
...
$q="select * from tb_partatlet where id_event='".$str->id."' limit ".$str->hal.",".$str->maxresult;
$sql=mysql_query($q);
if(mysql_num_rows($sql) > 0){
	...
	while($h=mysql_fetch_array($sql)){
		...
		$str->daftar.='
		  <tr>
			<td valign="top">'.$i.'</td>
			<td valign="top">'.$h['nama_alternatif'].'</td>
			<td valign="top" align="center">'.$h['satlat'].'</td>
			<td valign="top" align="center">'.$h['kelas'].'</td>
			
			<td align="center" valign="top"><a href="?'.$str->edit_link.'&amp;id='.$h['id_partatlet'].'&amp;action=delete"><img src="images/delete.png"></a> </td>
		  </tr>
		';
	}
}
	...
	<div style="width:96%; float:left; border:1px solid #999; padding:10px;">
		...
		<input name="txt_idevent" type="hidden" value="<?php echo $str->id;?>">
			<tr>
				...
				<td><input type="text"  id="namaevent" value="<?php echo $str->nama_event;?>" readonly style="font-size:13px;"/></td>
				<td style="font-size:14px;">Lokasi  </td>
				<td><input type="text" id="lokasi"  value="<?php echo $str->lokasi;?>" readonly style="font-size:13px;"/></td>
                <td style="font-size:14px;"><a href="?hal=tambah_partatlet">Kelola Tim</a> </td>
			</tr>
			<tr>
				<td style="font-size:14px;">Tahun</td>
				<td><input type="text" id="tahun" value="<?php echo $str->tahun;?>" readonly style="font-size:13px;"/></td>
			
				<td style="font-size:14px;">Tanggal</td>
				<td><input type="text" id="tanggal" value="<?php echo $str->tanggal;?>" readonly style="font-size:13px;" /></td>
			</tr>
      ...
      <div style="overflow:auto; max-height:250px; margin:10px 0 20px 0;" >
		...
		<table width="100%" border="0" cellspacing="4" cellpadding="0" class="tabel_reg">
		  <tr>
			...
			<td align="center" width="180">Nama Atlet</td>
